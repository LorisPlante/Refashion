  ...
  return (
    <header className="bg-background text-foreground shadow-md fixed top-0 z-50 w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        <Link href="/">
          <svg className="fill-foreground w-[148px] h-[40px]" xmlns="http://www.w3.org/2000/svg" width="529" height="100" fill="none" viewBox="0 0 529 100">
      ...
      {isOpen && (
        <div className="md:hidden bg-background text-foreground shadow-md">
          <nav className="flex flex-col items-start p-2 gap-2">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href} className="hover:text-blue-600 px-4 py-2" onClick={() => setIsOpen(false)}>
                {link.name}
              </Link>
            ))}
          </nav>
